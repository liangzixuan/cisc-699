...
- **W6 Hard Stop 3 validation harness added.** Added repeatable environment capture and validation workflow scripts, new Makefile targets (`env`, `validate`, `validate-docker`), and tests. Commit `674fa58` pushed so the Ubuntu droplet could clone an auditable version from GitHub.
- **Target-host validation completed on DigitalOcean droplet.** Droplet cloned commit `674fa58`; unit tests passed (`Ran 7 tests`). Target clean rerun passed local 12/12, Docker 6/6, gVisor 6/6, and API trace.
